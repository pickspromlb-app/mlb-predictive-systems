...
def profiles_today(profile_date: date | None = Query(default=None), window: str = 'L5'):
    d = profile_date or today_local()
    rows = fetch_all('''select p.*, t.abbreviation as team, o.abbreviation as opponent from propicks.daily_team_profile p left join core.teams t on t.team_id=p.team_id left join core.teams o on o.team_id=p.opponent_team_id where p.profile_date=%s and p.window=%s order by p.game_pk,p.home_away''', (d, window))
    return {'date': d, 'window': window, 'count': len(rows), 'rows': rows}
