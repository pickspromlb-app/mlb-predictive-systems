...
values ('PROHITS_MLB', 'ProHitsMLB', 'Player Hits', 'Sistema especializado únicamente en Over 0.5 hits de jugadores.', 'v1.0', true)
on conflict (system_id) do update set updated_at = now();

insert into propicks.filter_definitions (filter_id, system_id, filter_name, description, metric_a, operator_a, value_a, metric_b, operator_b, value_b, window, weight, required, status)
values
